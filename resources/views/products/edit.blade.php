...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css">
    ...
    <form action="{{ route('product.update', $product->id) }}" method="post" class="mx-auto max-w-5xl">
    @csrf
    @method("PUT")
    ...
    <div class="max-w-6xl mx-auto p-6 sm:px-3 lg:px-8">
        <div class="bg-white overflow-hidden sm:rounded-lg shadow-md">
            ...
            <div class="grid grid-cols-2 px-8 py-1">
                ...
                id="nama" name="nama" value="{{ $product->nama }}"/>
                @if($errors->has('nama'))
            ...
            <div class="grid grid-cols-2 px-8">
                <label for="kategori" class="my-4 pr-3 text-right">Pilih Kategori</label>
                    ...
                    <option value="{{ $k->id }}" @if($product->id_kategori==$k->id) selected @endif'>{{ $k->nama }}</option>
                    @endforeach
                    </select>
            </div>
            <div class="grid grid-cols-2 px-8">
                ...
                <input type="number" class="my-2 rounded-md border border-slate-400 text-sm shadow-sm
                        ...
                        id="qty" name="qty" value="{{ $product->qty }}"/>
                        @if($errors->has('qty'))
            ...
            <div class="grid grid-cols-2 px-8">
                ...
                <input type="number" step="100" class="my-2 rounded-md border border-slate-400 text-sm shadow-sm
                        ...
                        id="harga_beli" name="harga_beli" value="{{ $product->harga_beli }}"/>
            ...
            <label for="harga_jual" class="my-4 pr-3 text-right">Harga Jual</label>
                <input type="number" step="100" class="my-2 rounded-md border border-slate-400 text-sm shadow-sm
                        ...
                        id="harga_jual" name="harga_jual" value="{{ $product->harga_jual }}"/>
        ...
        <div class="grid sm:grid-cols-1 md:grid-cols-3">
            ...
            <button type="submit" class="mx-5 my-4 px-5 py-2 bg-gray-800 rounded-md text-white hover:bg-gray-600 focus:ring focus:ring-gray-300" value="Tambah Produk">Simpan Produk</button>
        </div>
    </div>
</div>
        </div>
